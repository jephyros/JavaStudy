...
public class F001_2751 {
    ...
    private static void fnHeapSort(int[] data) {
        ...
        for(int i = data.length;i>0;i--){
            ...
            data[0] = data[i-1];
            data[i-1] = tmp;
            fnMaxHeapMake(data,i -1);
        }

    }


    private static void fnMaxHeapMake(int[] data, int n) {
        int leftchild = n * 2 + 1;
        int rightchild = n * 2 + 2;

        //자식노드없으면 종료
        if ((data.length -1) < leftchild ){
            //아무것도안한다.
            return;
        }
        //자식이 하나만있으면(leftchild)
        else if((data.length -1) == leftchild){
            //부모보보다 자식이 크면 패스
